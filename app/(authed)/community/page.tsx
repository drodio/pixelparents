...
export default async function ExchangePage() {
    ...
    return shell(
      <>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-10 text-center">
          <h2 className="text-lg font-semibold">Verify to use the Community</h2>
          <p className="mx-auto mt-2 max-w-md text-sm text-white/55">
            {viewerSignup
              ? "Confirm your OHS student's Stanford email to post Asks and Offers and respond to the community."
              : "Your account isn't recognized as an OHS family yet. Join GoPixel to use the Community."}
          </p>
          <Link
    ...
    return shell(
      <>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-10 text-center text-white/55">
          The Community isn&apos;t available yet — check back soon.
        </div>
      </>,
    );
  }
  ...
  return shell(
    <>
      ...
      {posts.length === 0 ? (
        <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-10 text-center">
          <p className="text-white/60">Nothing on the Community yet.</p>
          <Link
            href="/community/new"
