...
Fixed the duplicate-profile bug where a founder with no GitHub who arrives via a second LinkedIn URL gets a "-2" twin. Diagnosed on Jordan Lee (/in/jlee + /in/jordan-lee-9239989a, both jlee.example, no github) and collapsed his two profiles in prod via mergeProfiles. Added a GitHub-less dedup key: name + same dedicated website.
...
- Tests: `tests/lib/identity-dedup.test.ts` (+ dedupWebsiteDomain, the Jordan Lee case, generic-host/no-website negatives). 16 pass.
- PROD DATA: merged jordan-lee-2 (loser) into jordan-lee (winner) via the existing mergeProfiles; the -2 slug is now an alias so the old URL still resolves.
