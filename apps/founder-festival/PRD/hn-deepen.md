...
`news.ycombinator.com/user?id=` URL, so they missed Sam Odio (#1 on the
...
- `src/lib/enrichers/hackernews.ts`: extracted a shared `resolveHnHandle(ctx,
  ...
  arbitrary, e.g. "Sam_Odio"). New exported helpers `registrableDomain`,
...
- Verified live: with NO Exa HN URL, `resolveHnHandle` discovers `Sam_Odio` via
