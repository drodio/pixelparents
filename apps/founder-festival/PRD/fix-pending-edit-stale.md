...
Fixed the admin "edit pencil" on pending owner-edited score rows appearing not to save (e.g. Janice Williams Oliver — removed a trailing period, but reopening the editor still showed it). Root cause: the row's displayed text and the editor pre-fill both read the server `item.reason` prop, which only updates via `router.refresh()` — that races the just-committed write and is unreliable for client components, so reopening re-filled from the stale prop. The backend `modify` route persists correctly; this was a UI-reflection bug.
