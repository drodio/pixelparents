...
### Summary of changes since last update
Fixed the bug where attendees (e.g. Jon Staenberg + Kevin Liu on event 6e515b68) kept showing the admin "SCORING…" chip indefinitely even though scoring was done. Added a reaper to the `scoring-tick` cron that fails items orphaned in `scoring` by a dead worker, so the parent job can finalize and the chip clears. bd issue ff-8j3.
...
- One-time cleanup of the EXISTING stuck rows in prod (Jon/Kevin on event 6e515b68 and any others) is still pending an explicit go-ahead — the cron reaper will heal them within ~a tick once deployed, but a manual sweep clears them immediately. Prod DB write, so gated on confirmation.
