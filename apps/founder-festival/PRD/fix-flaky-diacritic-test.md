...
Stabilized the flaky `search-diacritic` "Ebru case" test (bd ff-3qw). Root cause:
the persistent CI test branch accumulates a `score:120` "Ebru Yildirim" row every
...
- `tests/lib/search-diacritic.test.ts`:
  - Query is now `Ebru Yıldırım ${slugSuffix}` — the suffix matches only the
    ...
    "Yıldırım" still must ASCII-fold to match the stored "yildirim" (AND-ed with
...
- Pre-existing accumulated "Ebru Yildirim" rows remain in the persistent CI test
