...
Made every leaderboard badge a clickable filter, restructured the sidebar (no "Filters" heading / no "Clear all" / no "Outcome" facet; the Badges section now lists the full taxonomy with live counts, most-common first), added a white removable active-filter **pills row** under the search box, fixed the badge "fit" row so it fills with badges then shows "+N more" (was collapsing to just "+9 more"), and tokenized search so "sam rivera" matches "Samuel … Rivera". Built on the `worktree-scoring-rubric` (#178) base. Industry remains the other agent's lane (see `docs/coordination/leaderboard.md`).
