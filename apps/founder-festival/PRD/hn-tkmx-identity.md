...
Diagnosed why Sam Odio still showed no HN/Tokenmaxxing signal in prod (his
...
- `src/lib/enrichers/hackernews.ts`: `resolveHnHandle` tier 4 `tkmx` —
  ...
  name, prefix-compatible first name, so "Samuel Odio" ↔ `hn_username` "Sam_Odio"),
  then confirm that hn_username against HN with bio corroboration. Exported the two
  matchers for testing.
- Verified live (no highlights): Sam → `Sam_Odio` via `tkmx` (karma 4506);
...
The rubric only changes NEW scores. Existing profiles (Sam, DROdio, Geoff, the
