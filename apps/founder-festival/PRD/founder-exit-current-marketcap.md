...
Fixed a data bug surfaced by Jensen Huang ranking #14: the `founder_exit` rubric
used a public company's market cap AT IPO, so NVIDIA was scored at its 1999 IPO cap
...
23 `founder_exit` rows use IPO-day caps. Companies that GREW (NVIDIA, Reddit,
...
- RESCORE-to-apply: existing rows don't change until re-scored (the model must fetch
  the current cap). Jensen stays #14 until his rescore — then jumps to ~#1 (NVIDIA
