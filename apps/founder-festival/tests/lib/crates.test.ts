...
describe("githubLoginsFromUrls", () => {
  it("extracts the bare login from profile and repo URLs", () => {
    expect(githubLoginsFromUrls(["https://github.com/BurntSushi"])).toEqual(["BurntSushi"]);
    expect(githubLoginsFromUrls(["https://github.com/tokio-rs/tokio"])).toEqual(["tokio-rs"]);
  });
