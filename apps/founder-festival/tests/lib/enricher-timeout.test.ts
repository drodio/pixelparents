...
// withEnricherTimeout caps each enricher: on deadline (or rejection) it resolves
// to a KEPT result that carries status:"error" and no facts — so the source still
// shows in the roster as "errored / timed out" instead of silently disappearing,
// while the orchestrator's ok-only fact filter still ignores it. The eval proceeds
// with whatever completed in time.
...
describe("withEnricherTimeout", () => {
  ...
  it("resolves to an 'error' result when the enricher exceeds the deadline", async () => {
    const slow = new Promise<EnrichmentResult>((res) => setTimeout(() => res(real), 200));
    const r = await withEnricherTimeout("sec-edgar", slow, 20);
    expect(r).toEqual({ source: "sec-edgar", status: "error", note: "timed out", facts: [], citations: [] });
  });

  it("swallows a rejection into an 'error' result (never throws)", async () => {
    const r = await withEnricherTimeout("npm", Promise.reject(new Error("boom")), 100);
    expect(r).toEqual({ source: "npm", status: "error", note: "boom", facts: [], citations: [] });
  });
});
